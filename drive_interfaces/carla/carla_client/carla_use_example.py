...
def print_pack(measurements,i,write_images=False): # You can set here to save images
...
def use_example(ini_file,port = 2000, host ='127.0.0.1',print_measurements =False):

	""" Initialize CARLA. For this you must call the constructor with both host and port
		On this point, the client connects to the server to start the configuration procedure.
	"""
	...
	while True:
		try:
			"""
				User get the measurements from the server. 
			...
			if print_measurements:
				print_pack(measurements,i)

			"""
				Sends a control command to the server
				This control command contains the following fields:
				* throttle : goes from 0-1
...
if __name__ == "__main__" :
	...
	parser.add_argument("-pm", "--print_measurements", help="activate log and put the log file to screen",action="store_true")
	...
	use_example(args.config,port=args.port, host=args.host,print_measurements=args.print_measurements)
