...
def create_structure(tf, input_image,input_data, input_size,dropout,config):
	...
	with tf.name_scope("Speed"):
		...
		speed = network_manager.fc_block(speed,64)
		speed = network_manager.fc_block(speed,16)

	""" Joint sensory """
	j = tf.concat([x,speed],1)
	j = network_manager.fc_block(j, 128)
	...
	for i in range(0,len(config.branch_config)):  
		with tf.name_scope("Branch_" + str(i)):
			if config.branch_config[i][0] == "Speed":
				...
				branch_output =network_manager.fc_block(x,64)
				branch_output =network_manager.fc_block(branch_output,64)
			else:
				branch_output =network_manager.fc_block(j,64)
				branch_output =network_manager.fc_block(branch_output,64)
