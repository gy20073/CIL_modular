...
class DatasetManager(object):
    def __init__(self, config):
        ...
        self._images_val, self._datasets_val = self.read_all_files(config.val_db_path, config.sensor_names,
                                                                   config.dataset_names)
        ...
        self._splited_keys_val = spliter.split_by_output(self._datasets_val[0][config.variable_names.index("Steer")][:],
                                                         divided_keys_val)
        ...
        print(
        len(self._splited_keys_train[0][0]), len(self._splited_keys_train[0][1]), len(self._splited_keys_train[0][2]))

        self.train = Dataset(self._splited_keys_train, self._images_train, self._datasets_train, config, config.augment)
        self.validation = Dataset(self._splited_keys_val, self._images_val, self._datasets_val, config,
                                  [None] * len(config.sensor_names))
