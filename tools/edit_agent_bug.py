...
if __name__ == "__main__":
    ...
    positions_to_test = list(
        range(0, len([name for name in os.listdir(path) if os.path.isfile(os.path.join(path, name))])))
    ...
    for h_num in positions_to_test:
        ...
        if np.amax(data['labels'][0]) < 32:
            ...
            for i in range(0, 200):
                ...
                data['labels'][i] = join_classes(data['labels'][i], classes_join) * int(
                    255 / (number_of_seg_classes - 1))
