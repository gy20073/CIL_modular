...
class configMain:
    def __init__(self):
        ...
        self.branch_config = [['Steer', 'Gas', 'Brake']] * 4 + \
                            [["Speed"]]
        ...
        self.camera_middle_zoom = {'CameraMiddle': True}
...
class configInput(configMain):
    def __init__(self):
        ...
        all_files = glob.glob("/data/yang/code/aws/scratch/carla_collect/steer103_v5_dir*/*/data_*.h5")
        self.val_db_path = []
        for valid in range(1, 15, 3):
            self.val_db_path += glob.glob(
                "/data/yang/code/aws/scratch/carla_collect/steer103_v5_dir*/*WeatherId=" + str(valid).zfill(
                    2) + "/data_*.h5")
