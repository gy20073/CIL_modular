...
if __name__ == "__main__":
  ...
  path = '/home/muellem/Downloads/Desktop/RC28_wpz_M_DR/'
  path_dir = '/home/muellem/Downloads/Desktop/RC28_wpz_M_DR_WP/'
  ...
  for h_num in sequence_num:
    ...
    if (not(h_num in exception_list)):
		...
		target_array = data['targets'][i]
	      ...
	      wp1_vector, wp1_mag = get_vec_dist(wp1_x_player,wp1_y_player, loc_x_player, loc_y_player)
              if wp1_mag>0:
		wp1_angle = get_angle (wp1_vector, [ori_x_player,ori_y_player]) - target_array[camera_angle_ind]*math.pi/180
	      else:
		wp1_angle = 0

	      wp2_vector, wp2_mag = get_vec_dist(wp2_x_player,wp2_y_player, loc_x_player, loc_y_player)
              if wp2_mag>0:
		wp2_angle = get_angle (wp2_vector, [ori_x_player,ori_y_player]) - target_array[camera_angle_ind]*math.pi/180
	      else:
		wp2_angle = 0
