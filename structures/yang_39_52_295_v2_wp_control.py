    ...
    for i in range(0, len(config.branch_config)):
        with tf.name_scope("Branch_" + str(i)):
            if config.branch_config[i][0] == "Speed":
                ...
                branch_output = network_manager.fc_block(branch_output, 256)
                wps = network_manager.fc(branch_output, len(config.branch_config[i]) - 3)
                controls = network_manager.fc(wps, 3)
                branches.append(tf.concat((controls, wps), 1))
                print("stacked control of waypoints and control")
