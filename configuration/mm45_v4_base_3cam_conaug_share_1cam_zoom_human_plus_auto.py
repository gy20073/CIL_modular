...
class configMain:
    def __init__(self):
        ...
        # TODO: make sure it works correctly for more than 1 seg GT, especially the random number for apply or not
        # TODO: make the better architecutre of concateting the channels
...
class configInput(configMain):
    def __init__(self):
        ...
        # human demo
        all_files = glob.glob("/scratch/yang/aws_data/carla_collect/human/human_v1/CARLA_TOWN03/*/data_*h5")
        val_db_path = glob.glob("/scratch/yang/aws_data/carla_collect/human/human_v1/CARLA_TOWN03/b0004/data_*h5")
        train_db_path = list(set(all_files) - set(val_db_path))

        # auto demo
        all_files = glob.glob("/data/yang/code/aws/scratch/carla_collect/steer103_v2_dir/*/data_*.h5")
        self.val_db_path = []
        for valid in range(1, 15, 3):
            self.val_db_path += glob.glob(
                "/data/yang/code/aws/scratch/carla_collect/steer103_v2_dir/*WeatherId=" + str(valid).zfill(
                    2) + "/data_*.h5")
        self.train_db_path = list(set(all_files) - set(self.val_db_path))

        # merge them
        self.val_db_path += val_db_path
