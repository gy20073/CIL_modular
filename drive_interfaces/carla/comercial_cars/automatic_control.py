    ...
    def parse_events(self, client, world, clock):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    key = raw_input("the parameter key")
                    print(key)
                    value = float(raw_input("the parameter value"))
                    global agent
                ...
                if isinstance(self._control, carla.VehicleControl):
                    if event.key == K_q:
                        ...
                        world.player.set_autopilot(self._autopilot_enabled)
                        # turn off the original side autopilot, and switch to the python side
                        #print("temporaly turn off the autopilot functionality")
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                ...
                if sum(keys) > 0:
                    print("some key is presseddddd", keys)
                    ids = np.where(keys)
                    for id in ids:
                        name = pygame.key.name(id)
                        print("name of the key is ", name)
                    self._parse_vehicle_keys(keys, clock.get_time())
                    self._control.reverse = self._control.gear < 0
                    world.player.apply_control(self._control)
            elif isinstance(self._control, carla.WalkerControl):
                print("walker control is activated")
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if isinstance(c, carla.VehicleControl):
            #print(c.throttle) the throttle is not always as set by the main program
            self._info_text += [
...
def game_loop(args):
    ...
    try:
        ...
        if args.agent == "Roaming":
            ...
            if list == []:
                spawn_point = carla.Transform(carla.Location(x=-11.5, y=-8.0, z=2.0))
            else:
                spawn_point = list[0]
