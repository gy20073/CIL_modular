...
def drive(experiment_name,drive_config,name = None,memory_use=1.0):
	...
	if drive_config.show_screen:
		...
		screen_manager.start_screen(drive_config.resolution,drive_config.aspect_ratio,drive_config.scale_factor) #[800,600]
	...
	try:
		while direction != -1:
			...
			#actions = driver.compute_action(images.rgb[drive_config.middle_camera],measurements.forward_speed) #rewards.speed
				actions = driver.compute_action([measurements['BGRA'][drive_config.middle_camera],measurements['Labels'][drive_config.middle_camera]],speed) #measurements.speed
			...
			if drive_config.show_screen:
				if drive_config.interface == "Carla" or drive_config.interface == "VirtualElektra":
					...
					dist_to_goal = math.sqrt(( measurements.goal[0]- measurements.position[0]) *(measurements.goal[0] - measurements.position[0]) + (measurements.goal[1] - measurements.position[1]) *(measurements.goal[1] - measurements.position[1]))
