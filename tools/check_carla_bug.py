...
path = '/home/fcodevil/Desktop/20171024_Weather8_1_clean/'
...
if __name__ == "__main__":
  ...
  for h_num in positions_to_test:
    ...
    for i in range(0,197,sensors['RGB']*4):
      ...
      for j in range(sensors['labels']):
        capture_time =time.time()
        labels[int(data['targets'][i +j][camera_id_position])] = np.array(data['labels'][i+j]).astype(np.uint8)
        #print ' Read Label time ',time.time() - capture_time
      for j in range(sensors['depth']):
        depths[int(data['targets'][i +j][camera_id_position])] = np.array(data['depth'][i+j]).astype(np.uint8)
