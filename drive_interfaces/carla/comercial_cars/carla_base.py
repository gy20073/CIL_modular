class CarlaBase(object):
    ...
    def get_sensor_data(self, goal_pos=None, goal_ori=None):
        ...
        if self.use_planner:
            ...
            if sldist([data.player_x, data.player_y], [self.positions[self._current_goal][0],
                                                       self.positions[self._current_goal][1]]) < self._dist_to_activate:
                self._current_goal = random.randint(0, len(self.positions))
                self._dist_to_activate = random.randint(100, 500)

            direction = self.planner.get_next_command(pos, ori, [self.positions[self._current_goal][0],
                                                                 self.positions[self._current_goal][1], 22], (1, 0, 0))
