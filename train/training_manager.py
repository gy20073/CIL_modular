...
class TrainManager(object):
        ...
        with tf.name_scope("Loss"):
            if hasattr(self._config, 'intermediate_loss'):
                self._loss, self._variable_error, self._variable_energy, self._image_loss, self._branch \
                    = self._loss_function(self._output_network, self._seg_network, self._targets_data,
                                          self._input_images[:, :, :, 3:4],
                                          self._input_data[self._config.inputs_names.index("Control")], self._config)
            else:
                self._loss, self._variable_error, self._variable_energy, self._image_loss, self._branch \
                    = self._loss_function(self._output_network, self._targets_data,
                                          self._input_data[self._config.inputs_names.index("Control")], self._config)
    ...
    def build_optimization(self):
        ...
        with tf.name_scope("Optimization"):
            ...
            if hasattr(self._config, 'segmentation_model_name'):
                train_vars = list(set(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)) - set(
    ...
    def run_train_step(self, batch_tensor, sess, i):
        ...
        self._feedDict.update(
            {self._variable_learning: decrease_factor * self._config.learning_rate, self._dout: self._config.dropout})
