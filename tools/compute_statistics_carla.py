...
def get_colisions(selected_matrix, header):
    ...
    while i < selected_matrix.shape[0]:
        if (selected_matrix[i, header.index('collision_gen')] - selected_matrix[
            (i - 10), header.index('collision_gen')]) > 40000:
            count_gen += 1
            i += 20
        i += 1
    ...
    i = 1
    while i < selected_matrix.shape[0]:
        if (selected_matrix[i, header.index('collision_car')] - selected_matrix[
            (i - 10), header.index('collision_car')]) > 40000:
            count_car += 1
            i += 30
        i += 1
    ...
    i = 1
    while i < selected_matrix.shape[0]:
        if (selected_matrix[i, header.index('collision_ped')] - selected_matrix[
                i - 5, header.index('collision_ped')]) > 30000:
            count_ped += 1
            i += 100
        i += 1
...
def get_out_of_road_lane(selected_matrix, header):
    ...
    while i < selected_matrix.shape[0]:
        ...
        if (selected_matrix[i, header.index('sidewalk_intersect')] - selected_matrix[
            (i - 10), header.index('sidewalk_intersect')]) > 0.3:
            count_road += 1
            i += 20
        if i >= selected_matrix.shape[0]:
            break

        if (selected_matrix[i, header.index('lane_intersect')] - selected_matrix[
            (i - 10), header.index('lane_intersect')]) > 0.4:
            count_lane += 1
            i += 20
    ...
    for file in files_parsed:
        ...
        for t in tasks:
            ...
            for w in weathers:
                ...
                task_data_matrix = data_matrix[np.logical_and(data_matrix[:, header.index('exp_id')] == t,
                                                              data_matrix[:, header.index('weather')] == w)]
                if compute_infractions:
                    task_reward_matrix = reward_matrix[
                        np.logical_and(reward_matrix[:, header_rewards.index('exp_id')] == float(t),
                                       reward_matrix[:, header_rewards.index('weather')] == float(w))]
                ...
                if compute_infractions:
                    ...
                    infractions = [lane_road[0] / km_run, lane_road[1] / km_run, colisions[0] / km_run,
                                   colisions[1] / km_run, colisions[2] / km_run]
