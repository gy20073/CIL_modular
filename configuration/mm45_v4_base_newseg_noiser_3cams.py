...
class configMain:
    def __init__(self):
        ...
        self.perception_num_replicates = {"det_COCO": -1, "det_TL": -1, "seg": 8, "depth": -1, "det_TS": -1}
        # debug
        ...
        if self.use_perception_stack:
            ...
            # debug
            self.feature_input_size = (39, 52*3, 54)
        else:
...
class configInput(configMain):
    def __init__(self):
        ...
        for valid in range(1, 15, 3):
            self.val_db_path += glob.glob("/data/yang/code/aws/scratch/carla_collect/noiser_direction/*WeatherId="+str(valid).zfill(2)+"/data_*.h5")
