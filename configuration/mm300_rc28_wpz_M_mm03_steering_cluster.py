...
class configMain:
	...
	def __init__(self):
		...
		# y , x
		self.image_size = (88,200,1)
		self.network_input_size = (88,200,1)
		self.variable_names = ['Steer','Gas','Brake','Hand_B','Reverse','Steer_N','Gas_N','Brake_N','Pos_X','Pos_Y','Speed',\
						...
						'Ori_Z','Control','Camera','Angle','wp1_x','wp1_y','wp2_x','wp2_y','wp1_angle','wp1_mag','wp2_angle','wp2_mag']
		# _N is noise, Yaw_S is angular speed


		self.sensor_names=['labels']
		self.sensors_size = [(88,200,1)]
		...
		self.branch_config = [['wp1_angle','wp2_angle',"Steer","Gas","Brake"],['wp1_angle','wp2_angle',"Steer","Gas","Brake"],\
		['wp1_angle','wp2_angle',"Steer","Gas","Brake"],['wp1_angle','wp2_angle',"Steer","Gas","Brake"],["Speed"]]
		...
		self.seg_network_gt_one_hot_join = True
		self.number_of_labels = 2
		self.labels_mapping = {0:0,1:0,2:0,3:0,5:0,12:0,9:0,11:0,4:0,10:0,8:0,6:1,7:1}
...
class configInput(configMain):
	...
	def __init__(self,path='/'):
		...
		self.number_images_epoch = 1*3600*10*3	
...
class configTrain(configMain):
	...
	def __init__(self):
		...
		self.variable_weight = {'wp1_angle':0.1,'wp2_angle':0.1,'Steer':0.5,'Gas':0.2,'Brake':0.1,'Speed':1.0}
...
class configOutput(configMain):
	...
	def __init__(self):
		...
		self.targets_to_print = ['wp1_angle','wp2_angle','Steer','Gas','Brake'] # Also prints the error. Maybe Energy
