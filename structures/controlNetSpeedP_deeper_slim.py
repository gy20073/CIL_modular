...
def create_structure(tf, x, input_data, input_size, dropout, config):
    ...
    print(x)
    x = network_manager.conv_block(x, 3, 1, 16, padding_in='VALID')
    print(x)

    """conv2"""
    x = network_manager.conv_block(x, 3, 2, 32, padding_in='VALID')
    print(x)
    ...
    j = network_manager.fc_block(j, 256)
    ...
    for i in range(0, len(config.branch_config)):
        with tf.name_scope("Branch_" + str(i) + "_" + config.branch_config[i][0]):
            if config.branch_config[i][0] == "Speed":
                ...
                branch_output = network_manager.fc_block(x, 128)
                branch_output = network_manager.fc_block(branch_output, 128)
            else:
                branch_output = network_manager.fc_block(j, 128)
                branch_output = network_manager.fc_block(branch_output, 128)
