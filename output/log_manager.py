...
class LogManager(object):
    ...
    def print_screen_track(self, i, duration):
        ...
        if self._curses:
            ...
            terminal_draw_tool.draw_line(1, ['Epoch: ', ' Step: ', ' Images P/Sec: ', ' STATUS: '],
                                         [str(self._epoch_number), str(i), str(self._current_velocity), 'TRAINING'],
                                         [1, 1, 1, 2])
            terminal_draw_tool.draw_line(2, ['best TRAIN: ', ' on iter: ', ' best VAL: ', ' on iter: ', ' Last Loss: '],
                                         [str(self._lowest_error), str(self._lowest_iter), str(self._lowest_val_error),
                                          str(self._lowest_val_iter), str(self._last_train_error)], [2, 1, 2, 1, 1])
            ...
            print(
            "Epoch: %s, Step: %s, Images/Sec: %s" % (str(self._epoch_number), str(i), str(self._current_velocity)))
            print("Best Train: %s, on iter: %s, Best Val: %s, on iter: %s, Last Loss: %s" % (
            str(self._lowest_error), str(self._lowest_iter), str(self._lowest_val_error), str(self._lowest_val_iter),
            str(self._last_train_error)))
    ...
    def write_variable_error(self):
        ...
        for i in self._config.inputs_to_print:
            self._last_batch_inputs.append(
                feedDict[self._training_manager._targets_data[self._config.targets_names.index(i)]])
        ...
        for i in self._config.targets_to_print:
            ...
            targets = self._sess.run(self.split_target[self._config.targets_to_print.index(i)][index_target],
                                     feed_dict=feedDict)

            self._last_batch_targets.append(targets)
            self._last_batch_gts.append(
                feedDict[self._training_manager._targets_data[self._config.targets_names.index(i)]])
