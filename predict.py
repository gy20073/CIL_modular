    ...
    try:
        for h_num in positions_to_test:
            ...
            for i in range(0, 198, 3):
                ...
                actions_pred = driver.compute_action(images[drive_config.middle_camera], data['targets'][i][51],
                                                     data['targets'][i + 2][8])
                    ...
                    if drive_config.interface == "Carla":
                        for j in range(drive_config.number_screens):
                            screen_manager.plot_driving_interface(capture_time, np.copy(images[j]), \
                                                                  actions[j], action_noisy, rewards.direction,
                                                                  recording and (drifting_time == 0.0 or will_drift), \
                                                                  drifting_time, will_drift, rewards.speed, 0, 0, i)  #

                    elif drive_config.interface == "GTA":

                        dist_to_goal = math.sqrt(
                            (rewards.goal[0] - rewards.position[0]) * (rewards.goal[0] - rewards.position[0]) + (
                            rewards.goal[1] - rewards.position[1]) * (rewards.goal[1] - rewards.position[1]))
                        ...
                        screen_manager.plot_driving_interface(capture_time, np.copy(images), action, action_noisy, \
                                                              ...
                                                              , rewards.speed, 0, 0, None, rewards.reseted,
                        ...
                        for key, value in drive_config.cameras_to_plot.items():
                            screen_manager.plot_driving_interface(capture_time, np.copy(images[key]), \
                                                                  actions[key], actions_pred[key],
                                                                  data['targets'][i + 2][8], 0, \
                ...
                if drive_config.type_of_driver == "Machine" and drive_config.show_screen and drive_config.plot_vbp:
                    image_vbp = driver.compute_perception_activations(images[drive_config.middle_camera],
                                                                      data['targets'][i][51])
