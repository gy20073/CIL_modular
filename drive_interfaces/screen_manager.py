...
class  ScreenManager(object):
	...
	def set_array(self,array,screen_position,position=(0,0),scale=None):
		...
		pygame.surfarray.blit_array( self._camera_surfaces[screen_position[0]*screen_position[1]], array.swapaxes(0,1))

		camera_scale = pygame.transform.scale(self._camera_surfaces[screen_position[0]*screen_position[1]],(int(self._resolution[0]*scale),int(self._resolution[1]*scale)))
	...
	def plot_driving_interface(self,capture_time,sensor_data,\
		...
		self.paint_on_screen(size_x/10,'GAS',(0,255,0),( int(1.5*sensor_data.shape[0]/8) -20,initial_y_pos),screen_position)

		self.paint_on_screen(size_x/10,'BRAKE',(255,0,0),( int(1.5*sensor_data.shape[0]/8) +60,initial_y_pos),screen_position)
