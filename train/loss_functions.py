        ...
        for j in range(len(branches_configuration[i])):
            ...
            if i == 0 and j == 0:
                ...
                loss_function = tf.add(
                    variable_loss * config.branch_loss_weight[i] * config.variable_weight[target_name], loss_function)
...
def mse_seg_branched(network_outputs, seg_output, ground_truths, image_seg_ground_truth, control_input, config):
    ...
    for i in range(len(branches_configuration)):
        ...
        for j in range(len(branches_configuration[i])):
            ...
            energy_branch.append(variable_loss)
            if i == 0 and j == 0:
                loss_function = variable_loss * config.branch_loss_weight[i] * config.variable_weight[
                    target_name] + image_loss
                loss_control = variable_loss * config.branch_loss_weight[i] * config.variable_weight[target_name]
            else:
                loss_function = tf.add(
                    variable_loss * config.branch_loss_weight[i] * config.variable_weight[target_name], loss_function)
                loss_control = tf.add(
                    variable_loss * config.branch_loss_weight[i] * config.variable_weight[target_name], loss_control)
