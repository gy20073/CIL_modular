...
def create_structure(tf, input_image, input_data, input_size, dropout, config):
    ...
    for i in range(0, len(config.branch_config)):
        with tf.name_scope("Branch_" + str(i)):
            # each branch is a  classification head, plus a regression head
            ...
            this_out = [network_manager.fc(branch_output, config.waypoint_num_shapes)]

            # the waypoint_scale output
            branch_output = network_manager.fc_block(x, 256)
            branch_output = network_manager.fc_block(branch_output, 256)
            this_out.append(network_manager.fc(branch_output, 1))

            branches.append(this_out)
